...
namespace NS_Selidar
{
  ...
  bool Serial::bind(const char * portname, unsigned int baudrate,
                    unsigned int flags)
  {
  ...
  bool Serial::open(const char * portname, unsigned int baudrate,
                    unsigned int flags)
  {
    if(isOpened())
      close();
...
// FIXME: non-block io should be used
  ...
  int Serial::waitfordata(size_t data_count, unsigned int timeout,
